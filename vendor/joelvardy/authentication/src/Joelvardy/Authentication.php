...
class Authentication {
  ...
  protected function generate_hash($password, $salt = null) {
     ...
     * Generate hash
     *
     * @param  string [$hash] The password for which the hash should be generated for
     * @param  string [$salt] The salt should be the current password, if none is provided one will be generated
     * @return  string
     */
    protected function generate_hasher($hash, $salt = null) {
   ...
   */
  public function create_user($username, $password, $group_id, $firstname, $lastname,$mella, $dofb, $sex,$phone, $address, $town, $lga, $state, $affil_id, $active, $block, $hash) {
    ...
    $password = $this->generate_hash($password);
        $hash = $this->generate_hasher($hash);

    // Add database record
    if ( ! $stmt = Database::instance()->prepare("insert into {$this->config->user_table}( `group_id`, `created`, `updated`, `{$this->config->username_field}`, `{$this->config->password_field}`,`firstname`, `lastname`,`extramail`, `dofb`, `sex`,`phone`, `address`, `town`, `lga`,`state`, `affil_id`,`active`, `block`, `hash`) values ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)")) return false;
    $stmt->bind_param('iiisssssssssssssiis', $group_id, $_SERVER['REQUEST_TIME'], $_SERVER['REQUEST_TIME'], $username, $password, $firstname, $lastname,$mella, $dofb, $sex,$phone, $address, $town, $lga, $state, $affil_id, $active, $block, $hash);
  ...
  public function read_user($user_id) {
    ...
    public function read_user_details($username) {

        // Fetch cached copy of user details and return
        if ($user_details = Cache::fetch('user_'.$username)) return $user_details;

        // Read details from database
        if ( ! $stmt = Database::instance()->prepare("select * from {$this->config->user_table} where `{$this->config->username_field}` = ?")) return false;
        $stmt->bind_param('s', $username);
        $stmt->execute();
        $stmt->bind_result($id, $group_id, $created, $updated, $username, $password, $firstname, $lastname, $dofb, $sex, $phone, $address, $town, $lga, $state, $affil_id, $active, $block, $hash, $image, $mella, $bank, $accountnum, $accountname, $notes);
        $stmt->fetch();
        $user_details = array(
            'id' => $id,
            'group_id' => $group_id,
        ...
        // Cache user details
        Cache::store('user_'.$username, $user_details);
  ...
  public function login($user_id, $password) {
    ...
    if ( ! $stmt = Database::instance()->prepare("select `id`, `password` from {$this->config->user_table} where `id` = ?")) return false;
    $stmt->bind_param('i', $user_id);
    $stmt->execute();
	...
	public function loginSM($user_id, $password) {
  ...
  public function signIn($username, $password) {
    ...
    if ( ! $stmt = Database::instance()->prepare("select `username`, `password` from {$this->config->user_table} where `username` = ?")) return false;
    $stmt->bind_param('s', $username);
    $stmt->execute();
    $stmt->bind_result($username, $database_password);
